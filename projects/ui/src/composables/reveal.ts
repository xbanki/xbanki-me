...
interface IOptions {
     ...
     *
     *  - `EHook.INTERSECT` — The effect is applied when the target element is visible within the constraining box.
     *                        It is important to note, that regardless of the initially specified value, the state
     *                        variable will directly reflect whether the element is on screen or not.
     *  - `EHook.CUSTOM`    — A fully hands-off hook, which leaves the state management up to the caller via the `state`
     *                        reference variable.
     *  - `EHook.LOAD`      — Invokes the effect after the browser has finished loading the page, with a short delay
     *                        applied. It is important to note that the load hook flips the state variable boolean when
     *                        the hook is invoked.
     */
    hook: Hook,
...
function effectDefault(
    ...
    switch (state.value) {
        case true:
            if (el.value.classList.contains(inactive))
                el.value.classList.remove(inactive);
        ...
        case false:
            ...
            if (!el.value.classList.contains(inactive))
                el.value.classList.add(inactive);
 ...
 * @return           A new or pre-existing observer.
 */
    ...
    switch (typeof effect) {
        case 'object':
            ...
            for (const cls of DEFAULT_CLASSES)
                if (!el.value.classList.contains(cls))
        ...
        case 'function':
            if (once) {
                ...
                watch(state, () => effect(state, el), { immediate: true });
...
function createIntersectionHook(
    ...
    const { observer, targets } = createOrGetObserver(opts, (entries) => {
        const sorted = entries.length <= 1 ? entries : entries.sort(
            (a, b) => a.time - b.time,
        );
 ...
 *   2. `state` - Reference to the current reveal state. If the `options.hook` parameter is set to `custom`, this
 *                variable must be mutated *directly* to invoke the attached effect.
 *
...
export function useReveal(options?: DeepPartial<IOptions>) {
    ...
    onMounted(() => {
        ...
        switch (opts.hook) {
            ...
            case EHook.LOAD:
                createLoadHook(opts.delay, () => state.value = !state.value);
                createEffect(el as ShallowRef<HTMLElement>, state, opts.effect, opts.once);
