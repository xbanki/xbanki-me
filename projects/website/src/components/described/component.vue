<!--
 // Described component declaration.
 //
...
<template>
    ...
    <div class="sticky w-fit h-fit top-0" ref="v-described-title">
        <slot name="title"/>
    </div>
    ...
    <div class="transition-opacity duration-150 w-fit h-24" ref="v-described-content"
         v-bind:style="style">
        <slot name="default"/>
    </div>
</template>
 ...
 * @return         How much the elements overlap. `0` signifies none, `100` signifies complete overlap.
 */
function calculateOverlap(title: HTMLElement, content: HTMLElement) {
    ...
    return Math.min(
        Math.max(bounds_content.top + Math.floor(bounds_title.height / 3) - bounds_title.bottom, 0),
        1
    ) * 100;
}
